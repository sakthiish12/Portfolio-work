...
#include "PauseRestartThroneScript.h"
...
PauseRestartThroneScript::PauseRestartThroneScript(const char* scriptName) :
	scriptBase(scriptName)
{
}

void PauseRestartThroneScript::init(float dt, unsigned int idToBeSet)
{
...
void PauseRestartThroneScript::update(float dt)
{
	UNREFERENCED_PARAMETER(dt);
	if (ecs.entityHas<spriteCom>(id)) {
		...
		if (UiLayer::GetInstance().m_SceneState == UiLayer::SceneState::Pause && UiLayer::GetInstance().dialoguePause == false) {
			...
			if (MouseManager.isButtonPressed(MouseButton::LEFT_BUTTON) && UiLayer::GetInstance().pauseQuitConfirmation != true &&
				...
				mousePos.y <= (oTransform.worldMatrix[3][1] + oTransform.worldMatrix[1][1] / 2.0f) && InputManager.mouseClickEnabled)
			{
...
void PauseRestartThroneScript::end(float dt)
{
	UNREFERENCED_PARAMETER(dt);
	cout << "end()" << endl;
}

std::shared_ptr<scriptBase> PauseRestartThroneScript::createCopy()
{
	return std::make_shared <PauseRestartThroneScript>();
}

std::string PauseRestartThroneScript::getScriptName()
{
	return name;
}
