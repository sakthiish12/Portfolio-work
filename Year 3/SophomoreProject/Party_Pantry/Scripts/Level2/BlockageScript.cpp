// /******************************************************************************/
// /*!
// \file         BlockageScript.cpp
// \project name PartyPantry
// \author(s)    Poh Chun Ren, 100%
// \date         18th March, 2022
// \brief        Script to destory boulder as interactive object upon suction by player or shoot buy player.
// \lines        
//
...
BlockageScript::BlockageScript(const char* scriptName) :
	scriptBase(scriptName)
{
}

void BlockageScript::init(float dt, unsigned int idToBeSet)
{
...

	{
		...
		EC.emitterContainer.insert(std::make_pair("EmitterExplosion", std::make_shared<EmitterExplosion>()));
		EmitterExplosion* emi = dynamic_cast<EmitterExplosion*>(EC.emitterContainer["EmitterExplosion"].get());
		if(emi)
			emi->_newParticleProperties.textureId = "Cave Stone 5.png";
...
void BlockageScript::update(float dt)
{
	...
	else if (_state == BUSH_STATE::BeingSucked_Floating)
	{
		if (InSuctionRange == false)
		{
			...
			//disabling recovery because it can cause player to knock out of map



			//killing if player release because need to avoid collision issue
			if (ecs.entityHas<EmitterComponent>(id))
			{
				EmitterExplosion* emi = dynamic_cast<EmitterExplosion*>(ecs.getComponent<EmitterComponent>(id).emitterContainer["EmitterExplosion"].get());
				if (emi)
		...
		else
		{
			...
			constexpr float minimumShrinkDistance = 200.f; //kill bush
			...
			Force newF(Force::ForceType::LINEAR_FORCE, BushToPlayer * 8000.f, 1.f);
...
void BlockageScript::end(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
}

std::shared_ptr<scriptBase> BlockageScript::createCopy()
{
	return std::make_shared <BlockageScript>();
}

std::string BlockageScript::getScriptName()
{
	return name;
}
