...
#include "SpeedUpgradeDialogue.h"
...
using std::cout;
using std::endl;
SpeedUpgradeDialogueScript::SpeedUpgradeDialogueScript(const char* scriptName) :
	scriptBase(scriptName)
{
}

void SpeedUpgradeDialogueScript::init(float dt, unsigned int idToBeSet)
{
...
void SpeedUpgradeDialogueScript::update(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	if (ecs.entityHas<transform2D>(id))
	{
		...
		if (playerTransform.pos.x > (oTransform.pos.x - oTransform.scale.x / 2.f) && playerTransform.pos.x < (oTransform.pos.x + oTransform.scale.x / 2.f) &&
			...
			&& UpgradeManager::GetInstance().upgradeSuperSpeed) {

			dialogueActivated = true;
			//show this dialogue 
			entityID firstDialogueID = ecs.createEntityFromFile("Resources/PresetEntities/Level2SpeedUpgradeChatBubble.json");
...
void SpeedUpgradeDialogueScript::end(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	//cout << "end()" << endl;
}

std::shared_ptr<scriptBase> SpeedUpgradeDialogueScript::createCopy()
{
	return std::make_shared <SpeedUpgradeDialogueScript>();
}

std::string SpeedUpgradeDialogueScript::getScriptName()
{
	return name;
}
