...
#include "HUDSpeedOverlayScript.h"
...
HUDSpeedOverlayScript::HUDSpeedOverlayScript(const char* scriptName) :
	scriptBase(scriptName)
{
}

void HUDSpeedOverlayScript::init(float dt, unsigned int idToBeSet)
{
...
void HUDSpeedOverlayScript::update(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
...
void HUDSpeedOverlayScript::end(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	//cout << "end()" << endl;
}

std::shared_ptr<scriptBase> HUDSpeedOverlayScript::createCopy()
{
	return std::make_shared <HUDSpeedOverlayScript>();
}

std::string HUDSpeedOverlayScript::getScriptName()
{
	return name;
}
