...
TomatoCountScript::TomatoCountScript(const char* scriptName) :
	scriptBase(scriptName)
{
}

void TomatoCountScript::init(float dt, unsigned int idToBeSet)
{
...
void TomatoCountScript::update(float dt)
{
	if (ecs.entityHas<Com_Font>(id))
	{
		...
		//getting the number of tomatoes collected
		if (AssetManager::GetInstance().tomatoesCollected != prevTomatoesCollected) {
			oFont.text = std::to_string(AssetManager::GetInstance().tomatoesCollected);//update text
			prevTomatoesCollected = static_cast<int>(AssetManager::GetInstance().tomatoesCollected);//update previous collected 
			effectTrigger = true;
		}
...
void TomatoCountScript::end(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
}

std::shared_ptr<scriptBase> TomatoCountScript::createCopy()
{
	return std::make_shared <TomatoCountScript>();
}

std::string TomatoCountScript::getScriptName()
{
	return name;
}
