...
#include "UpgradeDialogueSpeedUnlockedScript.h"
...
using std::cout;
using std::endl;
UpgradeDialogueSpeedUnlockedScript::UpgradeDialogueSpeedUnlockedScript(const char* scriptName) :
	scriptBase(scriptName)
{
}

void UpgradeDialogueSpeedUnlockedScript::init(float dt, unsigned int idToBeSet)
{
...
void UpgradeDialogueSpeedUnlockedScript::update(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	if (ecs.entityHas<transform2D>(id))
	{
		...
		if (playerTransform.pos.x > (oTransform.pos.x - oTransform.scale.x / 2.f) && playerTransform.pos.x < (oTransform.pos.x + oTransform.scale.x / 2.f) &&
			...
			&& UpgradeManager::GetInstance().showUpgradeTutorial && UpgradeManager::GetInstance().upgradeSuperSpeed && UpgradeManager::GetInstance().partUpgradeSpeedTutorial) {

			dialogueActivated = true;
			//show this dialogue 
			entityID firstDialogueID = ecs.createEntityFromFile("Resources/PresetEntities/UpgradeChatBubbleSpeed.json");
...
void UpgradeDialogueSpeedUnlockedScript::end(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	//cout << "end()" << endl;
}

std::shared_ptr<scriptBase> UpgradeDialogueSpeedUnlockedScript::createCopy()
{
	return std::make_shared <UpgradeDialogueSpeedUnlockedScript>();
}

std::string UpgradeDialogueSpeedUnlockedScript::getScriptName()
{
	return name;
}
