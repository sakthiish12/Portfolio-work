// /******************************************************************************/
// /*!
// \file         UpgradeDialogueUnlocked3Script.cpp
// \project name Engine
...
#include "UpgradeDialogueUnlocked3Script.h"
...
using std::cout;
using std::endl;
UpgradeDialogue3UnlockedScript::UpgradeDialogue3UnlockedScript(const char* scriptName) :
	scriptBase(scriptName)
{
}

void UpgradeDialogue3UnlockedScript::init(float dt, unsigned int idToBeSet)
{
...
void UpgradeDialogue3UnlockedScript::update(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	if (ecs.entityHas<transform2D>(id))
	{
		...
		if (playerTransform.pos.x > (oTransform.pos.x - oTransform.scale.x / 2.f) && playerTransform.pos.x < (oTransform.pos.x + oTransform.scale.x / 2.f) &&
			...
			&& UpgradeManager::GetInstance().showUpgradeTutorial && UpgradeManager::GetInstance().upgradeSuperSpeed && UpgradeManager::GetInstance().partUpgradeSpeedTutorial3) {

			dialogueActivated = true;
			//show this dialogue 
			entityID firstDialogueID = ecs.createEntityFromFile("Resources/PresetEntities/UpgradeChatBubbleSpeed3.json");
			...
			UpgradeManager::GetInstance().partUpgradeSpeedTutorial3 = false;
			UpgradeManager::GetInstance().upgradeTutorial = true;//telling dialogue manager that it is upgrading dialogue 
		}
	}
}

void UpgradeDialogue3UnlockedScript::end(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	//cout << "end()" << endl;
}

std::shared_ptr<scriptBase> UpgradeDialogue3UnlockedScript::createCopy()
{
	return std::make_shared <UpgradeDialogue3UnlockedScript>();
}

std::string UpgradeDialogue3UnlockedScript::getScriptName()
{
	return name;
}
