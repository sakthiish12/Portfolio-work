// /******************************************************************************/
// /*!
// \file         UpgradeDialogueShield2Script.cpp
// \project name Engine
...
#include "UpgradeDialogueShield2Script.h"
...
using std::cout;
using std::endl;
UpgradeDialogueShield2Script::UpgradeDialogueShield2Script(const char* scriptName) :
	scriptBase(scriptName)
{
}

void UpgradeDialogueShield2Script::init(float dt, unsigned int idToBeSet)
{
...
void UpgradeDialogueShield2Script::update(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	if (ecs.entityHas<transform2D>(id))
	{
		...
		if (playerTransform.pos.x > (oTransform.pos.x - oTransform.scale.x / 2.f) && playerTransform.pos.x < (oTransform.pos.x + oTransform.scale.x / 2.f) &&
			...
			&& UpgradeManager::GetInstance().showUpgradeTutorial && UpgradeManager::GetInstance().upgradeSuperShield && UpgradeManager::GetInstance().partUpgradeShieldTutorial2) {

			dialogueActivated = true;
			//show this dialogue 
			entityID firstDialogueID = ecs.createEntityFromFile("Resources/PresetEntities/UpgradeChatBubbleShield2.json");
			...
			UpgradeManager::GetInstance().partUpgradeShieldTutorial2 = false;
			UpgradeManager::GetInstance().upgradeTutorial = true;//telling dialogue manager that it is upgrading dialogue 
		}
	}
}

void UpgradeDialogueShield2Script::end(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	//cout << "end()" << endl;
}

std::shared_ptr<scriptBase> UpgradeDialogueShield2Script::createCopy()
{
	return std::make_shared <UpgradeDialogueShield2Script>();
}

std::string UpgradeDialogueShield2Script::getScriptName()
{
	return name;
}
