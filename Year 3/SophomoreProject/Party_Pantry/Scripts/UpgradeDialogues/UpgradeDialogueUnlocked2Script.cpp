...
#include "UpgradeDialogueUnlocked2Script.h"
...
using std::cout;
using std::endl;
UpgradeDialogue2UnlockedScript::UpgradeDialogue2UnlockedScript(const char* scriptName) :
	scriptBase(scriptName)
{
}

void UpgradeDialogue2UnlockedScript::init(float dt, unsigned int idToBeSet)
{
...
void UpgradeDialogue2UnlockedScript::update(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	if (ecs.entityHas<transform2D>(id))
	{
		...
		if (playerTransform.pos.x > (oTransform.pos.x - oTransform.scale.x / 2.f) && playerTransform.pos.x < (oTransform.pos.x + oTransform.scale.x / 2.f) &&
			...
			&& UpgradeManager::GetInstance().showUpgradeTutorial && UpgradeManager::GetInstance().upgradeSuperSpeed && UpgradeManager::GetInstance().partUpgradeSpeedTutorial2 
			&& UpgradeManager::GetInstance().partUpgradeSpeedTutorial == false) {

			dialogueActivated = true;
			//show this dialogue 
			entityID firstDialogueID = ecs.createEntityFromFile("Resources/PresetEntities/UpgradeChatBubbleSpeed2.json");
			...
			UpgradeManager::GetInstance().partUpgradeSpeedTutorial2 = false;
			UpgradeManager::GetInstance().upgradeTutorial = true;//telling dialogue manager that it is upgrading dialogue 
		}
	}
}

void UpgradeDialogue2UnlockedScript::end(float dt)
{
	UNREFERENCED_PARAMETER(dt);//unref
	//cout << "end()" << endl;
}

std::shared_ptr<scriptBase> UpgradeDialogue2UnlockedScript::createCopy()
{
	return std::make_shared <UpgradeDialogue2UnlockedScript>();
}

std::string UpgradeDialogue2UnlockedScript::getScriptName()
{
	return name;
}
