...
static COORD		currentFontSize = {0,0};

static unsigned short bufferAttribute; /*Foreground Color, BackGround Color, Othereffects*/
...
void Console_SetRenderBuffer_Attribute(enum colorCode colorCodeFg, enum colorCode colorCodeBg, enum colorCode specialLVB) {
	bufferAttribute = colorCodeFg + (colorCodeBg << 4) + specialLVB;
}
...
/*Default is 8x8*/
void Console_SetSquareFont(int fontSize)
{	
	...
	currentFontSize.X = fontInfo.dwFontSize.X;
	currentFontSize.Y = fontInfo.dwFontSize.Y;
...
int Console_GetFontSizeX()
{
	return currentFontSize.X;
}

int Console_GetFontSizeY()
{
	return currentFontSize.Y;
}
...
void Console_SetSize(int x, int y)
{
	...
	const SMALL_RECT windowSize = { 0, 0, size.X - 1, size.Y - 1 };/*Define Console Window Size*/
...
int Console_GetWindowHorizontal() {
	GetWindowRect(consoleHWNDTrack, &consoleRectTrack);
	return (consoleRectTrack.right - consoleRectTrack.left);
}

int Console_GetWindowVertical() {
	GetWindowRect(consoleHWNDTrack, &consoleRectTrack);
	return (consoleRectTrack.bottom - consoleRectTrack.top);
}
